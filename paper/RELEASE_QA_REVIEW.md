...
| CI portability | The dedicated paper workflow is path-scoped, does not run v3/v4, explicitly installs `siunitx`, scalable EC fonts, and `rg` dependencies, and invokes the complete paper gate from a normal Git checkout; Linux Paper run `29991894931` and CI run `29991894903` passed with the audited deterministic-package payload |
