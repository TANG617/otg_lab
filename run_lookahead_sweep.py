...
OUTPUT_DIR = Path(__file__).parent / "results" / "lookahead_sweep"
...
def plot_sweep(rows):
    ...
    for row_index, target_mode in enumerate(TARGET_STATE_MODES):
        for column_index, (metric, label) in enumerate(metrics):
            ...
            axis.axvline(250, color="0.5", linewidth=0.9, alpha=0.6)
...
def plot_prediction_error(rows):
...
def plot_250_tracking(tracking_250, reference, original_count):
...
def main():
    ...
    sweep_plot = plot_sweep(rows)
    prediction_plot = plot_prediction_error(rows)
    tracking_plot = plot_250_tracking(tracking_250, reference, original_count)
