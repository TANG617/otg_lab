...
_METHOD_IDENTITY_SUFFIXES = {
    "deployed_p_only_ordinary_ruckig": "native ordinary Ruckig; unshielded",
    "predicted_p_ordinary_ruckig": "native ordinary Ruckig; unshielded",
    "raw_predicted_pv_ordinary_ruckig": "native ordinary Ruckig; unshielded",
    "raw_predicted_pva_ordinary_ruckig": "native ordinary Ruckig; unshielded",
    "deployed_p_only_ruckig_shielded": "Ruckig + viability shield; mixed allowed",
    "predicted_p_ruckig_shielded": "Ruckig + viability shield; mixed allowed",
    "predicted_pva_ruckig_shielded": "Ruckig + viability shield; mixed allowed",
    "one_step_governed_p_direct": "native one-step/direct",
    "one_step_governed_pv_direct": "native one-step/direct",
    "one_step_governed_pva_direct": "native one-step/direct",
    "jerk_qp_pva_direct": "native QP/direct",
    "jerk_qp_pva_ruckig": "QP + shielded Ruckig; mixed allowed",
    # Frozen v3 method IDs are intentionally labelled as the exposed mixed
    # identities if historical tables are viewed; their artifacts are not
    # rewritten or regenerated by this module.
    "deployed_p_only": "frozen v3 mixed Ruckig/one-step fallback",
    "predicted_p": "frozen v3 mixed Ruckig/one-step fallback",
    "raw_predicted_pv": "frozen v3 mixed Ruckig/one-step fallback",
    "scalar_projected_pva": "frozen v3 mixed Ruckig/one-step fallback",
}


def method_figure_label(method_id: str) -> str:
    """Return a display label that never hides native/shielded/mixed identity."""

    suffix = _METHOD_IDENTITY_SUFFIXES.get(str(method_id))
    return str(method_id) if suffix is None else f"{method_id} [{suffix}]"
    ...
    *,
    ...
    with deterministic_style():
        ...
        for color_index, series in enumerate(sorted(groups)):
            ...
            label = method_figure_label(series) if series_field == "method" else series
...
def _plot_complete_heatmap(
    ...
    *,
    ...
    with deterministic_style():
        ...
        column_labels = (
            [method_figure_label(value) for value in column_values]
            if column_field == "method"
            else column_values
        )
        row_labels = (
            [method_figure_label(value) for value in row_values]
            if row_field == "method"
            else row_values
        )
        axis.set_xticks(
            np.arange(len(column_values)), column_labels, rotation=35, ha="right"
        )
        axis.set_yticks(np.arange(len(row_values)), row_labels)
...
def plot_same_information_ablation(
    ...
    with deterministic_style():
        ...
        axis.set_xticks(
            x,
            [method_figure_label(method) for method in methods],
            rotation=25,
            ha="right",
        )
        axis.set(
...
def plot_direct_vs_ruckig(
    ...
    if len(followers) < 2:
        raise FigureValidationError(
            "method-identity figure requires at least two follower identities"
        )
    ...
    with deterministic_style():
        ...
        for trajectory in sorted(paired):
            axis.plot(
                np.arange(len(followers)),
                [paired[trajectory][follower] for follower in followers],
                color="#888888",
                alpha=0.35,
                linewidth=0.8,
            )
        ...
        axis.set_xticks(np.arange(len(followers)), followers, rotation=15, ha="right")
        axis.set(
            ylabel="raw-time position RMSE",
            title="Native, shielded, and direct method identity",
        )
        axis.legend(frameon=False)
        return save_deterministic_figure(figure, output)
    ...
    with deterministic_style():
        ...
        for index, method in enumerate(methods):
            axis.bar(
                ...
                width,
                label=method_figure_label(method),
                color=_PALETTE[index % len(_PALETTE)],
            )
    ...
    *,
    ...
    with deterministic_style():
        ...
        for axis, choice in zip(axes[:, 0], choices):
            ...
            identity = (
                method_figure_label(ranking_method)
                if ranking_method is not None
                else "single-method trace"
            )
            axis.set(
                ylabel="position",
                title=f"{choice['selection_reason']}: {trajectory_id} — {identity}",
            )
